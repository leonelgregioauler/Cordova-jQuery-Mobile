	  ...
	  if (contacts.length) {
		 ...
		 for(var i=0; i<length; ++i) {
            ...
            if(contacts[i].name) {
			  ...
			  appendList += '<div role="main" class="ui-content" data-role="content">' + 
			                   '<ul id="contactList" data-role="listview" data-inset="true">' + 
							      '<li class="contact_list_item" id="' + contact.id + '"><a href="#contact-info">' + contact.name.formatted + ' (' + contact.id + ')</a></li>' +
							   '</ul>' +
							'</div>';
	...
	onPageChange: function(event, data) {
      ...
      switch(toPageId) {
        case 'contact-info':
		  ...
		  $('#familyName').html(app.contactInfo.name.familyName);
		  $('#phone').html(app.contactInfo.phoneNumbers[0].value);
		break;
      }
    },
